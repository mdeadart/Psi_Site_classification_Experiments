...
def generate_kmers(line,k,alphabet={'A':'A','C':'C','G':'G','U':'U'}):
    read = ''
    l = line.strip('\n')
    M = len(l) - k + 1
    for n in range(M):
...
def embed_reads(path_sample,path_totalkmers,path_model,path_out,k=None,
    ...
    openFile = open(path_sample)
    fastaSequences_sqio = SeqIO.parse(openFile, "fasta")
    fastaSequences = [fasta for fasta in fastaSequences_sqio]
    total_reads = len(fastaSequences)
    
    #total_reads = sum([1 for fasta in fastaSequences])
    #openFile = open(path_sample)
    ...
    for fasta in fastaSequences:
        ...
        # datta added
        read_id, line = fasta.id, str(fasta.seq)
